...
class ContractManager:
    ...
    def send_token(self, owner: str, unique_hash: str, media_url: str) -> str:
        ...
        tx = self.contract.functions.mint(owner, unique_hash, media_url).build_transaction(
            {"nonce": nonce}
        )
        signed_tx = self.w3.eth.account.sign_transaction(tx, private_key=self.wallet_private_key)
